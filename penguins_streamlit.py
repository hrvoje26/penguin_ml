...
st.title('Penguin Classifier: Machine Learning App')
st.write("This app uses 6 inputs to predict the species of penguin using a model \
	built on the Palmer's Penguin's dataset. Use the form below to get started!")
...
if penguin_file is None:
	...
	st.write('We trained a Random Forest model on these data, it has a score of {}!\
		Use the inputs below to try out the model.'.format(score))
...
st.write('We used a machine learning (Random Forest) model to \
	predict the species, the features used in this preditction are ranked\
	by relative importance below.')
st.image('feature_importance.png')

st.write('Below are the histograms for each continous variable seperated by\
	penguin species. The vertical line represents your the inputed value.')
